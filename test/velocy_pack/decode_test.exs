defmodule VelocyPack.DecodeTest do
  ...
  describe "decode/2 for atoms" do
    test "decodes nil" do
      assert Decode.decode(<<0x18>>) === {:ok, nil}
    end

    test "decodes nil with tail" do
      assert Decode.decode(<<0x18, 0>>) === {:ok, nil, <<0>>}
    end
    ...
    test "decodes false with tail" do
      assert Decode.decode(<<0x19, 0>>) === {:ok, false, <<0>>}
    end
    ...
    test "decodes true with tail" do
      assert Decode.decode(<<0x1A, 0>>) === {:ok, true, <<0>>}
    end
  end

  describe "decode/2 for bit strings" do
    test "decodes empty string" do
      assert Decode.decode(<<0x40>>) === {:ok, ""}
    end

    test "decodes empty string with tail" do
      assert Decode.decode(<<0x40, 0>>) === {:ok, "", <<0>>}
    end
    ...
    test "decodes short string with tail" do
      assert Decode.decode(<<0x41, "a", 0>>) === {:ok, "a", <<0>>}
      assert Decode.decode(<<0x43, "foo", 0>>) === {:ok, "foo", <<0>>}

      string =
        "asdlfkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdfasdflkajsdflkj" <>
          "asdfasdfasdfasdfasdfalka"

      assert Decode.decode(<<0xBE, string::binary, 0>>) === {:ok, string, <<0>>}
    end
    ...
    test "decodes long string with tail" do
      string =
        "asdlfkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasd" <>
          "flkjasdflkajsdflkjasdflkjasdlksjadflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdflkjasdf" <>
          "lkjasd"
      ...
      assert Decode.decode(binary) === {:ok, string, <<0>>}
    end
  end

  describe "decode/2 for integers" do
    ...
    test "decodes small positive integers with tail" do
      assert Decode.decode(<<0x30, 0>>) === {:ok, 0, <<0>>}
      assert Decode.decode(<<0x31, 0>>) === {:ok, 1, <<0>>}
      assert Decode.decode(<<0x32, 0>>) === {:ok, 2, <<0>>}
      assert Decode.decode(<<0x33, 0>>) === {:ok, 3, <<0>>}
      assert Decode.decode(<<0x34, 0>>) === {:ok, 4, <<0>>}
      assert Decode.decode(<<0x35, 0>>) === {:ok, 5, <<0>>}
      assert Decode.decode(<<0x36, 0>>) === {:ok, 6, <<0>>}
      assert Decode.decode(<<0x37, 0>>) === {:ok, 7, <<0>>}
      assert Decode.decode(<<0x38, 0>>) === {:ok, 8, <<0>>}
      assert Decode.decode(<<0x39, 0>>) === {:ok, 9, <<0>>}
    end
    ...
    test "decodes small negative integers with tail" do
      assert Decode.decode(<<0x3A, 0>>) === {:ok, -6, <<0>>}
      assert Decode.decode(<<0x3B, 0>>) === {:ok, -5, <<0>>}
      assert Decode.decode(<<0x3C, 0>>) === {:ok, -4, <<0>>}
      assert Decode.decode(<<0x3D, 0>>) === {:ok, -3, <<0>>}
      assert Decode.decode(<<0x3E, 0>>) === {:ok, -2, <<0>>}
      assert Decode.decode(<<0x3F, 0>>) === {:ok, -1, <<0>>}
    end
    ...
    test "decodes signed integers with tail" do
      assert Decode.decode(<<0x20, 249, 0>>) === {:ok, -7, <<0>>}
      assert Decode.decode(<<0x20, 128, 0>>) === {:ok, -128, <<0>>}

      assert Decode.decode(<<0x21, 127, 255, 0>>) === {:ok, -129, <<0>>}
      assert Decode.decode(<<0x21, 0, 128, 0>>) === {:ok, -32_768, <<0>>}

      assert Decode.decode(<<0x22, 255, 127, 255, 0>>) === {:ok, -32_769, <<0>>}
      assert Decode.decode(<<0x22, 0, 0, 128, 0>>) === {:ok, -8_388_608, <<0>>}

      assert Decode.decode(<<0x23, 255, 255, 127, 255, 0>>) === {:ok, -8_388_609, <<0>>}
      assert Decode.decode(<<0x23, 0, 0, 0, 128, 0>>) === {:ok, -2_147_483_648, <<0>>}

      assert Decode.decode(<<0x24, 255, 255, 255, 127, 255, 0>>) === {:ok, -2_147_483_649, <<0>>}
      assert Decode.decode(<<0x24, 0, 0, 0, 0, 128, 0>>) === {:ok, -549_755_813_888, <<0>>}

      assert Decode.decode(<<0x25, 255, 255, 255, 255, 127, 255, 0>>) === {:ok, -549_755_813_889, <<0>>}
      assert Decode.decode(<<0x25, 0, 0, 0, 0, 0, 128, 0>>) === {:ok, -140_737_488_355_328, <<0>>}

      assert Decode.decode(<<0x26, 255, 255, 255, 255, 255, 127, 255, 0>>) === {:ok, -140_737_488_355_329, <<0>>}
      assert Decode.decode(<<0x26, 0, 0, 0, 0, 0, 0, 128, 0>>) === {:ok, -36_028_797_018_963_968, <<0>>}

      assert Decode.decode(<<0x27, 255, 255, 255, 255, 255, 255, 127, 255, 0>>) ===
               {:ok, -36_028_797_018_963_969, <<0>>}

      assert Decode.decode(<<0x27, 0, 0, 0, 0, 0, 0, 0, 128, 0>>) === {:ok, -9_223_372_036_854_775_808, <<0>>}
    end
    ...
    test "decodes unsigned integers with tail" do
      assert Decode.decode(<<0x28, 10, 0>>) === {:ok, 10, <<0>>}
      assert Decode.decode(<<0x28, 255, 0>>) === {:ok, 255, <<0>>}

      assert Decode.decode(<<0x29, 0, 1, 0>>) === {:ok, 256, <<0>>}
      assert Decode.decode(<<0x29, 255, 255, 0>>) === {:ok, 65_535, <<0>>}

      assert Decode.decode(<<0x2A, 0, 0, 1, 0>>) === {:ok, 65_536, <<0>>}
      assert Decode.decode(<<0x2A, 255, 255, 255, 0>>) === {:ok, 16_777_215, <<0>>}

      assert Decode.decode(<<0x2B, 0, 0, 0, 1, 0>>) === {:ok, 16_777_216, <<0>>}
      assert Decode.decode(<<0x2B, 255, 255, 255, 255, 0>>) === {:ok, 4_294_967_295, <<0>>}

      assert Decode.decode(<<0x2C, 0, 0, 0, 0, 1, 0>>) === {:ok, 4_294_967_296, <<0>>}
      assert Decode.decode(<<0x2C, 255, 255, 255, 255, 255, 0>>) === {:ok, 1_099_511_627_775, <<0>>}

      assert Decode.decode(<<0x2D, 0, 0, 0, 0, 0, 1, 0>>) === {:ok, 1_099_511_627_776, <<0>>}
      assert Decode.decode(<<0x2D, 255, 255, 255, 255, 255, 255, 0>>) === {:ok, 281_474_976_710_655, <<0>>}

      assert Decode.decode(<<0x2E, 0, 0, 0, 0, 0, 0, 1, 0>>) === {:ok, 281_474_976_710_656, <<0>>}
      assert Decode.decode(<<0x2E, 255, 255, 255, 255, 255, 255, 255, 0>>) === {:ok, 72_057_594_037_927_935, <<0>>}

      assert Decode.decode(<<0x2F, 0, 0, 0, 0, 0, 0, 0, 1, 0>>) === {:ok, 72_057_594_037_927_936, <<0>>}

      assert Decode.decode(<<0x2F, 255, 255, 255, 255, 255, 255, 255, 255, 0>>) ===
               {:ok, 18_446_744_073_709_551_615, <<0>>}
    end
  end

  describe "decode/2 for lists" do
    test "decodes empty list" do
      assert Decode.decode(<<0x01>>) === {:ok, []}
    end

    test "decodes empty list with tail" do
      assert Decode.decode(<<0x01, 0>>) === {:ok, [], <<0>>}
    end
    ...
    test "decodes compact list with tail" do
      data =
        <<0x13, 0xC1, 0x01, 0x14, 0xBD, 0x01, 0x44, 0x6E, 0x61, 0x6D, 0x65, 0x43, 0x66, 0x6F, 0x6F, 0x49, 0x70, 0x61,
          0x72, 0x61, 0x6D, 0x65, 0x74, 0x65, 0x72, 0x0A, 0x45, 0x76, 0x61, 0x6C, 0x75, 0x65, 0xBF, 0x96, 0x00, 0x00,
          0x00, 0x00, 0x00, 0x00, 0x00, 0x46, 0x4F, 0x52, 0x20, 0x62, 0x6C, 0x61, 0x68, 0x62, 0x6C, 0x61, 0x68, 0x62,
          ...
          0x0A, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x7B, 0x79,
          0x61, 0x64, 0x64, 0x61, 0x79, 0x61, 0x64, 0x64, 0x61, 0x79, 0x61, 0x64, 0x64, 0x61, 0x2C, 0x20, 0x62, 0x6C,
          0x61, 0x68, 0x62, 0x6C, 0x61, 0x68, 0x62, 0x6C, 0x61, 0x68, 0x7D, 0x03, 0x01, 0>>
      ...
      assert Decode.decode(data) === {:ok, list, <<0>>}
    end

    # todo - test decoding lists
  end

  describe "decode/2 for maps" do
    test "decodes empty map" do
      assert Decode.decode(<<0x0A>>) === {:ok, %{}}
    end

    test "decodes empty map with tail" do
      assert Decode.decode(<<0x0A, 0>>) === {:ok, %{}, <<0>>}
    end
    ...
    test "decodes compact map with tail" do
      data =
        <<0x14, 0xCD, 0x01, 0x47, 0x71, 0x75, 0x65, 0x72, 0x69, 0x65, 0x73, 0x13, 0xC1, 0x01, 0x14, 0xBD, 0x01, 0x44,
          0x6E, 0x61, 0x6D, 0x65, 0x43, 0x66, 0x6F, 0x6F, 0x49, 0x70, 0x61, 0x72, 0x61, 0x6D, 0x65, 0x74, 0x65, 0x72,
          0x0A, 0x45, 0x76, 0x61, 0x6C, 0x75, 0x65, 0xBF, 0x96, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46, 0x4F,
          ...
          0x20, 0x20, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x7B, 0x79, 0x61, 0x64, 0x64, 0x61, 0x79, 0x61, 0x64,
          0x64, 0x61, 0x79, 0x61, 0x64, 0x64, 0x61, 0x2C, 0x20, 0x62, 0x6C, 0x61, 0x68, 0x62, 0x6C, 0x61, 0x68, 0x62,
          0x6C, 0x61, 0x68, 0x7D, 0x03, 0x01, 0x01, 0>>
      ...
      assert Decode.decode(data) === {:ok, map, <<0>>}
    end
    ...
    test "decodes map with 1-byte offset without padding" do
      data =
        <<0x0B, 43, 3, 51, 76, 102, 111, 111, 47, 50, 49, 54, 50, 53, 49, 57, 53, 49, 72, 50, 49, 54, 50, 53, 49, 57, 53,
    ...
    test "decodes map with 1-byte offset without padding with tail" do
      data =
        <<0x0B, 43, 3, 51, 76, 102, 111, 111, 47, 50, 49, 54, 50, 53, 49, 57, 53, 49, 72, 50, 49, 54, 50, 53, 49, 57, 53,
          50, 75, 95, 86, 107, 51, 70, 65, 112, 87, 45, 45, 45, 3, 17, 27, 0>>
    ...
    test "decode2 map with 2-byte offset and zero-byte padding" do
      data =
        <<0x0C, 44, 1, 15, 0, 0, 0, 0, 0, 68, 117, 115, 101, 114, 24, 72, 100, 97, 116, 97, 98, 97, 115, 101, 71, 95, 115,
    ...
    test "decode2 map with 2-byte offset and zero-byte padding with tail" do
      data =
        <<0x0C, 44, 1, 15, 0, 0, 0, 0, 0, 68, 117, 115, 101, 114, 24, 72, 100, 97, 116, 97, 98, 97, 115, 101, 71, 95, 115,
          121, 115, 116, 101, 109, 67, 117, 114, 108, 76, 47, 95, 97, 100, 109, 105, 110, 47, 101, 99, 104, 111, 72,
          112, 114, 111, 116, 111, 99, 111, 108, 67, 118, 115, 116, 70, 115, 101, 114, 118, 101, 114, 11, 31, 2, 71, 97,
